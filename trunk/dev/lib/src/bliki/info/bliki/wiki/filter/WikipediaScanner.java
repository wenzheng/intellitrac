...
//	}
	...
	public StringBuilder replaceTemplateParameters(String template, Map<String, String> fTemplateParameters) {
		...
		try {
			...
			while (true) {
				...
				// else if (ch == '}' && (parameterStart > 0) && (fScannerPosition <
				// fSource.length - 1) && fSource[fScannerPosition] == '}'
				// && fSource[fScannerPosition + 1] == '}') {
				// List<String> list = splitByPipe(fSource, parameterStart,
				// fScannerPosition - 1, null);
				// if (list.size() > 0) {
				// String parameterString = list.get(0);
				// String value = null;
				// if (fTemplateParameters != null) {
				...
				// }
				// if (bufferStart < fScannerPosition) {
				// buffer.append(fSource, bufferStart, parameterStart - bufferStart -
				// 3);
				// }
				// buffer.append(value);
		...
		try {
			while (currOffset < endOffset) {
				...
				if (ch == '[' && srcArray[currOffset] == '[') {
					currOffset++;
					temp = findNestedEnd(srcArray, '[', ']', currOffset);
					if (temp >= 0) {
						currOffset = temp;
					}
					...
					if (srcArray[currOffset] == '{') {
						...
						temp = findNestedTemplateEnd(srcArray, currOffset);
						if (temp >= 0) {
							currOffset = temp;
						}
					}
				} else if (ch == '|') {
					value = new String(srcArray, lastOffset, currOffset - lastOffset - 1).trim();
			...
			if (currOffset > lastOffset) {
				resultList.add(new String(srcArray, lastOffset, currOffset - lastOffset).trim());
	...
	public static final int findNestedTemplateEnd(final char[] sourceArray, int startPosition) {
		...
		try {
			while (true) {
				...
				if (ch == '{' && sourceArray[position] == '{') {
					...
					if ((len > position) && sourceArray[position] == '{') {
						...
						if (temp < 0) {
							// return -1;
							position--;
							temp = findNestedTemplateEnd(sourceArray, position);
							if (temp < 0) {
								return -1;
							}
						}
				...
				} else if (ch == '}' && sourceArray[position] == '}') {
					// template ending
					position++;
					break;
				}
	...
	public static final int findNestedParamEnd(final char[] sourceArray, int startPosition) {
		...
		try {
			while (true) {
				...
				} else if (ch == '}' && sourceArray[position] == '}' && sourceArray[position + 1] == '}') {
	 ...
	 * @return
	 */
	protected final int findTemplateEnd(final char[] sourceArray, int startPosition) { // int
		// startPosition)
		// {
		char ch;
		int len = sourceArray.length;
		...
		// return findNestedTemplateEnd(fSource, startPosition);
	}
	 ...
	 * quotes (&amp;#34;) and single quotes (&amp;#39;). For doublequotes authors
	 * can also use the character entity reference &amp;quot;.
	 * <p>
...
//	protected final int readUntilIgnoreCase(Object processed, int start, String startString, String endString) {
//		int index = Utils.indexOfIgnoreCase(fBMHR, processed, fStringSource, startString, endString, start);
//		if (index != (-1)) {
//			return index + startString.length() + endString.length();
//		}
//		return -1;
